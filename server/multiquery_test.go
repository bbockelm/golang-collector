...
// runMultiQuery drives one QUERY_MULTIPLE_ADS against a running collector and returns the
// flat ad sequence, bucketed by MyType. The framing (PutInt32(1)+ad ... PutInt32(0)) is
// the same as a single-type query, so runQuery reads it directly.
...
// checkMultiQuery runs the multi-target query and asserts the negotiator-style result:
// one Machine (slot1, matched by the per-target Cpus>5 constraint, projected to Name+MyType
// so Memory is gone) and one full Submitter. Shared across the in-memory and remote-db
// backends so both multiQueryHandler paths (buffered iterator vs. store.Streamer) are
// covered by the same assertions.
func checkMultiQuery(t *testing.T, addr string) {
	t.Helper()
	// Per-target: Machine constrained to Cpus>5 and projected to Name+MyType; Submitter
	// unconstrained/unprojected. MyType is kept in the projection (as the negotiator does)
	// so the flat sequence can be bucketed by ad type.
...
// TestMultiQueryInMemory covers multiQueryHandler over the in-memory store (the buffered
// Query iterator branch): a multi-target query with per-target constraint + projection.
...
// exercises the store.Streamer (QueryStream) path -- each target's ads are streamed and
// relayed as they arrive instead of buffered. Same assertions as the in-memory case, so
// the two delivery paths are proven equivalent.
func TestMultiQueryRemoteDB(t *testing.T) {
	...
	if _, ok := interface{}(rpc).(store.Streamer); !ok {
		t.Fatal("RPCBackend must implement store.Streamer for this test to exercise the streaming path")
	}
